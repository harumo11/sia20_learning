...
int main(int argc, char* argv[])
{
    ...
    if (supervisor_data_csv.is_open()) {
        ...
        std::cout << "||| csv file collumn size : " << supervisor_data_csv.column_size() << std::endl;
        std::cout << "||| csv file raw size : " << supervisor_data_csv.row_size() << std::endl;
    } else {
    ...
    std::cout << "||| "
              << "x dim " << x_dim << " y_dim " << y_dim << std::endl;
    ...
    for (int epoch = 0; epoch < EPOCH_SIZE; epoch++) {
        for (int iteration = 0; iteration < ITERATION; iteration++) {
            //エポック数を表示
            std::cout << "||| "
                      << "epoch : " << epoch << " / " << EPOCH_SIZE << std::endl;
            //イテレーション数を表示
            std::cout << "||| "
                      << "iteration : " << iteration << " / " << ITERATION << std::endl;
